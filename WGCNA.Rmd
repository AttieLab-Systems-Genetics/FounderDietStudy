...
```{r}
plot(LivMetEigen, bysex = "F-M", volcano = TRUE, ylab = "eigen rank")
```

Compare eigentrait to traits

```{r}
object <- eigen_compare(LivMetModule, "F+M", "magenta", LivMetContr)
```
...
```{r}
plot(object, bysex = "F+M", volcano = TRUE, threshold = c(SD = 1, p = .1))
```
