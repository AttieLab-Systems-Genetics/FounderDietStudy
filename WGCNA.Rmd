...
LivMetEigen <- eigen_contrast(LivMetModule, LivMetContr) %>%
  mutate(p.value = 10^-(max(module) + 1 - module))
```
...
```{r}
plot(LivMetEigen, bysex = "F+M")
```

**Need to work on order of Y axis for module.**

```{r}
plot(LivMetEigen, bysex = "F-M", volcano = TRUE, ordername = "module")
```

Compare eigentrait to traits

```{r}
object <- eigen_traits(LivMetModule, "F+M", "magenta",
                       LivMetContr,
                       LivMetEigen %>% select(-p.value)) %>%
  mutate(p.value = 10 ^ -(1 - abs(pmin(0.99, kME))))
```

```{r}
plot(object, bysex = "F+M")
```

```{r}
summary_conditionContrasts(LivMetEigen %>% select(-p.value))
```

```{r}
plot(object %>% mutate(kME = abs(kME)), bysex = "F+M", volcano = TRUE, ordername = "kME")
```
     
```{r}
traitModule <- list(LivMet = LivMetModule)
saveRDS(traitModule, file.path(harmonizeddir, "traitModule.rds"))
```
...
EnrichContr <- conditionContrasts(EnrichSignal, EnrichStats,
                                  termname = "signal")
EnrichContrSignal <- EnrichContr %>%
  rename(cellmean = "value") %>%
  select(-p.value) %>%
  mutate(dataset = "Enrich",
         signal = cellmean)
```
...
EnrichTimes <- traitTimes(
        EnrichContrSignal, EnrichContrSignal,
        timetrait_selection[1], "cellmean", "minute")
```

```{r}
ggplot_traitTimes(EnrichTimes)
```


```{r}
ggplot_traitTimes(EnrichTimes, facet_strain = TRUE)
```
