...
The Lipid data has been updated, and the names have been aligned. To do:

- redeploy apps with updated Lipid data (traitX have been updated on RD)
- modify trait names in MixMod (module element?) to align
...
mixmodtraits <- 
  ...
  str_remove(mixmod$value$modules$trait, "_[0-9]+$")
...
Lipid traits revised by Holland (with additional summaries):

```{r}
holland <- readxl::read_excel(file.path(params$rootdir, "Primary data from Mark",
  "Founder liver lipid raw values from Holland used for mixed module calculation.xlsx"))
```

Number of new traits.

```{r}
LipidStats <- readRDS("Lipid/LipidStats.rds")
Lnames <- unique(LipidStats$trait)
```

```{r}
oldlipid <- filter(shinytraits, dataset == "Lipid")$trait
lipid <- names(select(holland, -(mouse_id:Batch)))
mlipid <- match(oldlipid, lipid, nomatch = 0)
c(match = sum(mlipid > 0), new = length(lipid[-mlipid]), nomatch = sum(mlipid == 0))
```
...
mliviso <- match(make.names(mixmodtraits), make.names(liviso), nomatch = 0)
nliviso <- match(make.names(liviso), make.names(mixmodtraits), nomatch = 0)
c(LivIso = sum(mliviso > 0),
  LivIso2 = sum(nliviso > 0),
  NA_iso = sum(mliviso[mixmodLivIso] == 0),
  NA_iso2 = sum(nliviso == 0))
```

Further checking of mismatch. For the `r sum(nliviso == 0)` isoforms that did not match exactly,
none of their gene names or isoform IDs match. These will be excluded.

```{r}
c(gene = sum(is.na(match(str_remove(liviso[nliviso == 0], "_[0-9]+$"), 
                mixmodtraits[mixmodLivIso]))),
ID = sum(is.na(match(str_remove(liviso[nliviso == 0], "^.*_"), 
                mixmodtraits[mixmodLivIso]))))
```

For the `r sum(mliviso[mixmodLivIso] == 0)` that are not included in the
...
```{r}
missIso <- mixmodtraits[mixmodLivIso][mliviso[mixmodLivIso] == 0]
...
Plan is to modify names in the Mixed Module to agree with trait data.
Need also to incorporate Holland's new traits.
