...
Plan is to modify names in the Mixed Module `mixmod$modules` to agree with trait data. Note that with trait as "dataset: trait", some modification of routines
will be necessary.
...
mixmod$modules <-
  MixModmodules |>
  mutate(
    trait = ifelse(dataset %in% c("LipidDrop","LivIsoDrop"),
                   NA, renameTrait),
    dataset = ifelse(dataset %in% c("LipidDrop","LivIsoDrop"),
                   NA, dataset)) |>
  select(dataset, trait, module, kME)
```
